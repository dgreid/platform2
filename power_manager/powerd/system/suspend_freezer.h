...
static constexpr char kFreezerStateFreezing[] = "FREEZING";
...
class SuspendFreezerInterface {
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(SuspendFreezerInterface);
};
...
class SuspendFreezer : public SuspendFreezerInterface {
 ...
 private:
  ...
  // Returns true if the deps are successfully populated, or if there are no
  // deps for the cgroup. Returns false if a dep for the cgroup doesn't exist.
  bool SetCgroupDeps(
  ...
  DISALLOW_COPY_AND_ASSIGN(SuspendFreezer);
};
